...
pub struct LaunchOptions<'a> {
    ...
    /// direct ip map
    #[builder(default = "None")]
    pub host_rules: Option<Vec<(String, String)>>,
}
...
impl Process {
    ...
    fn start_process(launch_options: &LaunchOptions) -> Result<TemporaryProcess> {
        ...
        let host_rules_option = if let Some(host_ip_tuple) = &launch_options.host_rules {
            Some(host_ip_tuple.iter().map(|tuple| {
                format!("MAP {} {}", tuple.0.as_str(), tuple.1.as_str())
        ...
        if let Some(rules) = host_rules_option {
            let host_rules = format!("--host-rules= {}", rules);
            args.push(Box::leak(host_rules.into_boxed_str()));
