...
use crate::protocol::network::methods::{EmulateNetworkConditions, SetExtraHTTPHeaders};
...
pub struct Tab {
    ...
    failure_handler: Arc<Mutex<Option<FailureHandler>>>,
...
impl Tab {
    ...
    fn start_event_handler_thread(&self) {
        ...
        let failure_handler_mutex = self.failure_handler.clone();
            ...
            for event in incoming_events_rx {
                ...
                match event {
                    ...
                    Event::LoadingFailed(ev) => {
                        if let Some(handler) = failure_handler_mutex.lock().unwrap().as_ref() {
                            let request_id = ev.params.request_id.clone();

                            match received_event_params.lock().unwrap().get(&request_id) {
                                Some(params) => handler(params.to_owned()),
                                _ => (),
                            }
                        }
                    }
    ...
    pub fn enable_failure_handling(&self, handler: FailureHandler) -> Result<()> {
        self.call_method(network::methods::Enable {})?;
        *(self.failure_handler.lock().unwrap()) = Some(handler);
        Ok(())
    }
    ...
    pub fn emulate_network_condition(&self, condition: EmulateNetworkConditions) -> Result<()> {
        self.call_method(condition)?;
        Ok(())
    }
